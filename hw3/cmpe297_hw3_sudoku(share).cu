...
__global__ void k_Sudoku(stContext *context)
{
	...
	while(!FINISHED())
	{
		...
		__shared__ float shared_val[WIDTH][WIDTH];
		__shared__ float shared_num_options[WIDTH][WIDTH];
		__shared__ float shared_not_in_cell[WIDTH][WIDTH];
		__shared__ float shared_not_in_row[WIDTH][WIDTH];
		__shared__ float shared_not_in_col[BLOCKSIZE][BLOCKSIZE];
		...
		if(share_num_options[row][col] > 1)
		{
					...
					if(share_num_options[row][col] == 1)
					{
						share_not_in_row[row][value] = 0;
						share_not_in_col[col][value] = 0;
						share_not_in_cell[(row)/3+((col)/3)*3][value] = 0;
						share_val[row][col] = value+1;
					}
		}
		__syncthreads();
	}//end while
