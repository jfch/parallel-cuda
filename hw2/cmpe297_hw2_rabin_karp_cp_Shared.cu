...
findIfExistsCu(char* input, int input_length, char* pattern, int pattern_length, int patHash, int* result, int *runtime)
{
	...
	__shared__ char inputStr[20];
	__shared__ char patternStr[10];

	inputStr[tid] = input[tid];
	if(tid<pattern_length)
		patternStr[tid] = pattern[tid];
	if(tid<input_length)
		inputStr[tid] = input[tid];
	if(pattern_length>input_length - pattern_length+1)
		for(int p=input_length - pattern_length+1; p<pattern_length;p++)
			patternStr[p] = pattern[p];
	if(input_length>input_length - pattern_length+1)
		for(int p=input_length - pattern_length+1; p<input_length;p++)
			inputStr[p] = input[p];
	
	for(int i = tid; i< tid+pattern_length; i++)
		inputHash = (inputHash*256 + inputStr[i]) % 997;
	if(inputHash == patHash && memcpy(inputStr,tid,patternStr,pattern_length)==0)
		result[tid]=1;
	else
		result[tid]=0;
