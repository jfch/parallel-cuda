...
__global__ void MatrixMulCUDA(float* A, float* B, float* C, unsigned long long *runtime)
{
	...
	for(int nIter=0; nIter<WIDTH/BLOCKSIZE; nIter++)
	{
		__shared__ float shared_A[BLOCKSIZE][BLOCKSIZE];
		__shared__ float shared_B[BLOCKSIZE][BLOCKSIZE];
		
		shared_A[row][col] = A[(blockRow*BLOCKSIZE+row) * WIDTH + (blockCol*BLOCKSIZE+col)];
		shared_B[row][col] = B[(blockRow*BLOCKSIZE+row) * WIDTH + (blockCol*BLOCKSIZE+col)];
		__syncthreads();

		for(int k = 0; k < BLOCKSIZE; k++)
		{
			cVal += shared_A[row][k] * shared_B[k][col] ;
		}
		__syncthreads();
	}

	C[(blockRow*BLOCKSIZE+row) * WIDTH + (blockCol*BLOCKSIZE+col)] = cVal;
...
main(void) 
{
    ...
    // --> 
    dim3 blocksPerGrid (WIDTH/BLOCKSIZE, WIDTH/BLOCKSIZE);// FILL HERE
    dim3 threadsPerBlock (BLOCKSIZE,BLOCKSIZE);// FILL HERE
	printf("CUDA kernel launch with %d blocks of %d threads\n", (WIDTH/BLOCKSIZE)*(WIDTH/BLOCKSIZE), BLOCKSIZE*BLOCKSIZE);

	MatrixMulCUDA<<<blocksPerGrid, threadsPerBlock>>>(d_A, d_B, d_C, d_runtime);
